...
class GradSLAMDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        # use unidepth
        self.use_unidepth = kwargs.get("use_unidepth", False)
        if self.use_unidepth:
            print("--------- basedataset.py: using MDE-estimated depth ----------")
        ...
        # print(f"selected color paths: {self.color_paths}")
    ...
    def __getitem__(self, index):
        ...
        # depth_path_gt = self.gt_depth_paths[index]
        # depth_path_unidepth = self.unidepth_depth_paths[index]
        depth_path = self.depth_paths[index]
        color = np.asarray(imageio.imread(color_path), dtype=float)
        color = self._preprocess_color(color)

        if not self.use_unidepth:
            # old code
            ...
            # load unidepth depth. H, W
            depth = np.load(depth_path)
            # preprocess as done by self._preprocess_depth
            depth = cv2.resize(
                depth,
                (self.desired_width, self.desired_height),
                interpolation=cv2.INTER_NEAREST,
