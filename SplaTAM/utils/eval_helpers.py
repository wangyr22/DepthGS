...
def eval(dataset, final_params, num_frames, eval_dir, sil_thres, 
    ...
    try:
        ...
        # modified: export to kitti format
        for name, w2c_list in [("output", latest_est_w2c_list), ("gt", gt_w2c_list)]:
            out = []
            for w2c in w2c_list:
                w2c = w2c.detach().cpu().numpy()
                c2w = np.linalg.inv(w2c).reshape(-1)
                out.append(
                    ' '.join(str(x) for x in c2w[:12]) + '\n')
            with open(os.path.join(eval_dir, f"{name}_pose.txt"), 'w') as f:
                f.writelines(out)
    except:
        ate_rmse = 100.0
